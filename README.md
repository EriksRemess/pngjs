...
- `fastFilter` - when `true` and `filterType` is auto or an array, picks one filter from the first scanline and reuses it for all scanlines (faster write, potentially larger output). Default: `true`
