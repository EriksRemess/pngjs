...
pub fn sync_read(input: Buffer, check_crc: Option<bool>) -> napi::Result<SyncReadResult> {
    ...
    let color_type = match decoded.color_type {
    ...
    Ok(SyncReadResult {
        ...
        depth: decoded.depth,
        colorType: decoded.color_type,
        interlace: decoded.flags & FLAG_INTERLACE != 0,
        palette: decoded.flags & FLAG_PALETTE != 0,
        color: decoded.flags & FLAG_COLOR != 0,
        alpha: decoded.flags & FLAG_ALPHA != 0,
        bpp: bpp_for_color_type(color_type),
        gamma: decoded.gamma_scaled as f64 / 100000.0,
        data: decoded.data.into(),
    })
}

#[allow(clippy::too_many_arguments)]
#[napi(js_name = "filterPack")]
pub fn filter_pack(
    data: Buffer,
    width: u32,
    height: u32,
