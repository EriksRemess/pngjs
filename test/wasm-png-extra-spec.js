...
wasmTest("CRC check can be disabled (sync and async)", async () => {
  let src = createPngObject(2, 2);
  let buf = PNG.sync.write(src);
  let corrupt = corruptIhdrCrc(buf);

  assert.throws(() => PNG.sync.read(corrupt), Error);
  ...
  await assert.rejects(parseAsync(corrupt), Error);
...
wasmTest("PNG.pack emits Error object when no data is provided", async () => {
  let png = new PNG();
...
wasmTest(
  "PNG.parse emits Error object when PNG has no IDAT chunks",
  async () => {
    let src = createPngObject(1, 1);
    let buf = PNG.sync.write(src);
    let noIdat = removeIdatChunks(buf);

    await assert.rejects(parseAsync(noIdat), Error);
  },
);

wasmTest("promise APIs reject on invalid input", async () => {
  await assert.rejects(new PNG().parse(Buffer.alloc(0)), Error);
...
wasmTest(
  "sync and async parsing produce identical output for a small fixture",
  async () => {
...
wasmTest(
  "sync and async parsing produce identical output for an interlaced fixture",
  async () => {
...
wasmTest("parser handles PNGs with multiple IDAT chunks", async () => {
