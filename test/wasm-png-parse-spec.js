...
wasmTest("should correctly parse an 8-bit grayscale png", async () => {
...
wasmTest("should correctly parse an 8-bit truecolor png", async () => {
...
wasmTest(
  "should correctly parse an 8-bit truecolor png with alpha",
  async () => {
...
wasmTest("should correctly read image with scanline filter", async () => {
...
wasmTest("should correctly read an indexed color image", async () => {
...
wasmTest(
  "should correctly read an indexed color image with alpha",
  async () => {
...
wasmTest("should correctly support crazily-filtered images", async () => {
...
wasmTest("should bail with an error given an invalid PNG", async () => {
  let buf = Buffer.from("I AM NOT ACTUALLY A PNG", "utf8");
  await assert.rejects(parseBuffer(buf), Error);
});

wasmTest("should bail with an error given an empty file", async () => {
  let buf = Buffer.from("");
  await assert.rejects(parseBuffer(buf), Error);
});

wasmTest("should bail with an error given a bad chunk type", async () => {
  await assert.rejects(parseFile("with_bad_type.png"), Error);
});

wasmTest("should bail with an error given a truncated PNG", async () => {
...
wasmTest(
  "should set alpha=true in metadata for images with tRNS chunk",
  async () => {
...
wasmTest("Should parse with low highWaterMark", async () => {
  await parseInputFile("tbbn0g04.png", { highWaterMark: 2 });
});

wasmTest("should support promise parse on instance", async () => {
...
wasmTest("should support static promise parse helpers", async () => {
  let buffer = fs.readFileSync(
    path.join(__dirname, "png-parse-data", "truecolor.png"),
  );
