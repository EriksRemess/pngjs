...
export default function bitPacker(dataIn, width, height, options) {
  ...
  let bytesPerSample = bitDepth16 ? 2 : 1;
  let fastPath =
    options.colorType === options.inputColorType &&
    (options.bitDepth === 8 || (bitDepth16 && IS_BIG_ENDIAN));
  if (fastPath) {
    let bpp = constants.COLORTYPE_TO_BPP_MAP[options.colorType];
    let expectedLength = width * height * bpp * bytesPerSample;
    ...
    // If no conversion is needed, return input as-is.
    return dataIn;
  }
  ...
  let expectedLength = width * height * inBpp * bytesPerSample;
  if (dataIn.length !== expectedLength) {
    throw new Error(
      "input data length mismatch: expected " +
        expectedLength +
